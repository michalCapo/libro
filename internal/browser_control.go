...
func RunBrowserMCP(in io.Reader, out io.Writer) error {
	...
	for scanner.Scan() {
		...
		case "tools/list":
			...
			reply["result"] = map[string]any{"tools": []any{map[string]any{"name": "browser", "description": browserHelp, "inputSchema": map[string]any{"type": "object", "properties": properties, "required": []string{"action"}, "additionalProperties": false}}}}
		case "tools/call":
