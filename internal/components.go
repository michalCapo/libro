...
func renderAppFrameBase(app Application, index int, selected bool, sid string, placeholder bool, zenMode ...bool) *r.Node {
	...
	zenCloseCls := "hidden"
	if zen {
		zenCloseCls = "absolute top-1 right-1 z-50 h-5 w-5 flex items-center justify-center pointer-events-none"
	}
	zenCloseBtn := r.Div(zenCloseCls).
		Attr("data-zen-close", "").
		Render(
			r.Button("flex h-5 w-5 items-center justify-center cursor-pointer pointer-events-auto opacity-0 rounded-sm border shadow-sm transition-[opacity,background-color,border-color,color] duration-150 ease-out hover:opacity-100 focus:opacity-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400/45").
				Attr("title", "Close app").
				Attr("aria-label", "Close app").
				Attr("style", "background:oklch(98% 0.012 24);border-color:oklch(88% 0.045 24);color:oklch(43% 0.16 24);").
				OnClick(r.JS(fmt.Sprintf("__ws.callSilent('app.close',{sid:%s,id:%s})", components.JSString(sid), components.JSString(app.ID)))).
				Render(r.I("material-icons-round text-[14px] leading-none block").Text("close")),
		)
...
(function(){
	...
	function navigateSelection(delta){
		...
		if(delta>0&&selIdx<max){selIdx++;render();}
	...
	function searchNormalKey(e){
		...
		if(e.key==='Escape'){
			e.preventDefault();e.stopImmediatePropagation();closeSearch();return;
		}
	}
	...
	document.addEventListener('keydown',searchNormalKey,true);
...
(function(){
	...
	function searchNormalKey(e){
		if(dlg.classList.contains('hidden')||searchPane.classList.contains('hidden'))return;
		if(document.activeElement===searchInput)return;
		if(e.key==='i'){
			e.preventDefault();e.stopImmediatePropagation();searchInput.focus();return;
		}
		if(e.key==='j'||e.key==='ArrowDown'){
			e.preventDefault();e.stopImmediatePropagation();navigateSelection(1);return;
		}
		...
		if(e.key==='Enter'){
			e.preventDefault();e.stopImmediatePropagation();copySelected('password');return;
		}
		if(e.key==='u'||e.key==='U'){
...
(function(){
	...
	function navigateSelection(delta){
		...
		if(delta>0)selectedIdx=Math.min(selectedIdx+1,filteredURLs.length-1);
