package components

import (
	r "github.com/michalCapo/g-sui/ui"
)

// WorktreeCreatePopup renders the popup used to create a new git worktree
// (and matching branch) from the current project's branch.
func WorktreeCreatePopup() *r.Node {
	return r.Div("fixed inset-0 z-[60] flex items-start justify-center pt-[15vh] bg-black/40 dark:bg-black/60 backdrop-blur-sm transition-opacity duration-75 hidden").
		ID(WorktreeCreatePopupID).
		OnClick(r.JS(HideJS(WorktreeCreatePopupID))).
		Render(
			r.Div("bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700/50 rounded-lg shadow-2xl w-full max-w-md mx-4 overflow-hidden").
				OnClick(r.JS("event.stopPropagation()")).
				Render(
					r.Div("px-4 py-3 border-b border-gray-200 dark:border-zinc-700/50 flex items-center gap-2").Render(
						r.I("material-icons-round text-blue-500 text-lg").Text("alt_route"),
						r.Span("text-sm font-medium text-gray-800 dark:text-zinc-200").Text("New Worktree"),
					),
					r.Div("px-4 py-3").Render(
						r.Div("text-[11px] font-mono text-gray-500 dark:text-zinc-500 mb-2").
							ID("worktree-create-context").
							Text(""),
						r.Input("w-full bg-transparent text-gray-800 dark:text-zinc-200 text-sm placeholder-gray-400 dark:placeholder-zinc-500 outline-none font-mono border border-gray-200 dark:border-zinc-700 rounded px-2 py-1.5").
							ID("worktree-create-input").
							Attr("type", "text").
							Attr("placeholder", "new-branch-name").
							Attr("autocomplete", "off").
							Attr("spellcheck", "false"),
					),
					r.Div("px-4 pb-3").Render(
						r.Div("text-[10px] font-mono uppercase tracking-wider text-gray-400 dark:text-zinc-600 mb-1").
							Text("Existing branches"),
						r.Div("max-h-56 overflow-y-auto border border-gray-100 dark:border-zinc-800 rounded").
							ID("worktree-create-branches"),
					),
					r.Div("px-4 py-2 border-t border-gray-100 dark:border-zinc-800 flex items-center gap-4 text-[10px] font-mono text-gray-400 dark:text-zinc-600").Render(
						r.Span("").Text("Enter create"),
						r.Span("").Text("Esc cancel"),
					),
				),
		)
}
