(function () {
  ...
  function applicationControl(command) {
    return new Promise((resolve, reject) => {
      ...
      applicationRequests.set(request, reply => {
        clearTimeout(timer);
  ...
  function applicationResult(request, reply) {
    const complete = applicationRequests.get(request);
    applicationRequests.delete(request);
    complete?.(reply);
  }
  ...
  function refresh() {
    observer.disconnect();
    const sidebar = document.getElementById('workspace-projects');
    if (sidebar) resizeHandle(sidebar, 'sidebar');
  ...
  function resizeHandle(host, kind) {
