...
Project threads contain one center agent panel each. Ctrl+N creates a new thread directly without opening the agent picker. Ctrl+Shift+A opens Replace agent, which starts a fresh session in the current thread and keeps its tools open. Issues and bottom terminal processes are shared across the project; browser and other tools belong to each thread. Project settings choose a shared application or one application per thread, with automatic ports and optional per-thread overrides. Standalone threads keep their existing behavior.
